# GAE patch for rails 2.3.5
# http://d.hatena.ne.jp/urekat/20091227/1261908269
# add this to before `Rails.boot!` in boot.rb
#   require "#{File.dirname(__FILE__)}/gae_boot_patch.rb"
          ...
          class GemDependency
            class << self
              def method_missing(m, *args)
                puts "Rails::GemDependency.\#{m}(\#{args.inspect}) is called."
              end
            end
          end
        end
  ...
  class Dir
    class << self
      ...
      def glob(*args, &block)
        ...
        p ["Dir.glob", args, ret] if ret.empty?
        ret
      end
      alias_method :__glob2_orig, :"[]"
      def [](*args, &block)
        ret = __glob2_orig(*args, &block)
        p ["Dir.glob2", args, ret] if ret.empty?
        ret
      end
    end
  end
